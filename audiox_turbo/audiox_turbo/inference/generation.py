...
from .sampling import sample, sample_k, sample_rf, sample_dmd
from ..data.utils import PadCrop

torch.backends.cudnn.benchmark = False # 固定算法 
torch.backends.cudnn.deterministic = True # 选确定性实现 
torch.backends.cuda.matmul.allow_tf32 = False # 关闭 TF32 
torch.backends.cudnn.allow_tf32 = False 
torch.set_float32_matmul_precision("highest") # FP32 走真 FP32 
...
@torch.no_grad()
def generate_diffusion_cond_dmd(
        model,
        steps: int = 4,
        conditioning: dict = None,
        conditioning_tensors: tp.Optional[dict] = None,
    ...
    """Few-step inference for the distilled AudioX-Turbo student.

    Unlike ``generate_diffusion_cond`` (multi-step, classifier-free guidance), this
    runs the DMD few-step sampler (default 4 steps, no CFG). The conditioning
    interface is identical, so the same ``conditioning`` dict works for T2A / V2A /
    V2M / TV2A.

    Args:
        model: the AudioX-Turbo ``diffusion_cond`` model (student weights loaded).
        steps: number of sampling steps (4 by default).
        conditioning: list with a single dict of multimodal conditions.
        sample_size: output length in audio samples.
        seed: random seed, or -1 for a random seed.
    ...
    model_dtype = next(model.model.parameters()).dtype
    noise = noise.type(model_dtype)
    conditioning_inputs = {k: (v.type(model_dtype) if torch.is_tensor(v) else v)
                           for k, v in conditioning_inputs.items()}

    sampled = sample_dmd(model.model, noise, steps=steps, device=device, **conditioning_inputs)
