...
@torch.no_grad()
def sample_dmd(model_fn, noise, steps=4, device="cuda", **extra_args):
    """Few-step DMD-distilled sampler used by AudioX-Turbo.

    The student is a rectified-flow model distilled with Distribution Matching
    Distillation, so it predicts a velocity ``v`` with ``z_t = (1 - t) * x0 + t * eps``.
    Each step estimates the clean latent ``x0 = z_t - t * v`` and re-noises it to the
    next timestep; the final step returns ``x0`` directly. No classifier-free
    guidance is used (the distilled student runs at cfg_scale = 1.0).

    Args:
        model_fn: the inner diffusion network (``model.model``).
        noise: initial latent noise of shape ``[B, C, T]`` (z at t = 1).
        steps: number of sampling steps (4 by default).
        extra_args: conditioning inputs from ``model.get_conditioning_inputs(...)``.
    """
    ...
    for i in range(steps):
        t = timesteps[i].expand(batch_size).to(device)
        pred_v = model_fn(z_t, t, **extra_args)
        t_expand = t.view(-1, 1, 1)
        pred_x0 = z_t - t_expand * pred_v
        if i < steps - 1:
            next_t = timesteps[i + 1].expand(batch_size).view(-1, 1, 1).to(device)
            fresh_noise = torch.randn_like(pred_x0)
            z_t = next_t * fresh_noise + (1 - next_t) * pred_x0
        else:
            final_latents = pred_x0
