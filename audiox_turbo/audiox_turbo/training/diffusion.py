...
from .losses import AuralossLoss, MSELoss, MultiLoss, CosineEmbeddingLoss
...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        if 'dasheng-loss' in optimizer_configs and optimizer_configs['dasheng-loss'].get('if_add_dasheng_loss', False):
            ...
            self.dasheng_feature_extractor = AutoFeatureExtractor.from_pretrained(model_path, trust_remote_code=True)
            self.dasheng_model = AutoModel.from_pretrained(model_path, outputdim=None, trust_remote_code=True)
            self.sample_rate = optimizer_configs['dasheng-loss']['config']['sample_rate']
            self.loss_modules = [
                MSELoss("output", 
                    "targets", 
                    weight=1.0 - dasheng_align_weight, 
                    mask_key="padding_mask" if self.mask_padding else None, 
                    name="mse_loss"
                ),
                CosineEmbeddingLoss("output", 
                    "target_feature", 
                    weight=dasheng_align_weight, 
                    margin=0.0, 
                    reduction="mean",
    ...
    def training_step(self, batch, batch_idx):
        ...
        reals, metadata = batch # reals.shape:[1, 2, 2097152]; (47s)
        # metadata: {'path': './data/-j9x-d4ZqtY.wav', 'relpath': '-j9x-d4ZqtY.wav', 'timestamps': [0.0, 1.0], 'seconds_start': 0, 'seconds_total': 6, 'padding_mask': tensor([1., 1., 1.,  ..., 0., 0., 0.], device='cuda:0'), 'load_time': 0.03414344787597656, 'prompt': '-j9x-d4ZqtY.wav'}]
        ...
        with torch.cuda.amp.autocast():
            ...
            p.tick("diffusion")
            # take_memory_snapshot("After diffusion")
            ...
            # 计算dasheng特征用于余弦嵌入损失
            if hasattr(self, 'dasheng_model') and hasattr(self, 'dasheng_feature_extractor'):
                with torch.no_grad():
                    # 对原始输入音频reals进行特征提取
                    # 将reals转换为dasheng模型期望的格式
                    if reals.shape[1] == 2:  # 立体声
                        # 转换为单声道
                        audio_input = reals.mean(dim=1)  # [batch, samples]
                    else:
                        audio_input = reals.squeeze(1)  # [batch, samples]
                    
                    # 使用dasheng特征提取器
                    inputs = self.dasheng_feature_extractor(audio_input.to('cpu'), sampling_rate=self.sample_rate, return_tensors="pt")
                    inputs = {k: v.to(self.device) for k, v in inputs.items()}
                    
                    # 获取dasheng特征
                    with torch.no_grad():
                        dasheng_outputs = self.dasheng_model(**inputs)
                        target_feature = dasheng_outputs.hidden_states.to(self.device)

                        # 调整 target_feature 到 [B, D, L]
                        if target_feature.ndim == 3:
                            target_feature = target_feature.transpose(1, 2)

                        # 同时在特征维(D)与时间维(L)上插值到与 output 匹配的尺寸
                        # 结果形状为 [B, output.shape[1], output.shape[2]]
                        # import pdb; pdb.set_trace()
                        target_feature = F.interpolate(
                            target_feature.unsqueeze(1),  # [B, 1, D, L]
                            size=(output.shape[1], output.shape[2]),
                            mode="bilinear",
                            align_corners=False,
                        ).squeeze(1)
                        # import pdb; pdb.set_trace()                        
                    loss_info["target_feature"] = target_feature
            loss, losses = self.losses(loss_info)
...
class DiffusionCondDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):        
        ...
        try:
            ...
            # pdb.set_trace()
            # conditioning['prompt'][0].shape:[4, 128, 768]
