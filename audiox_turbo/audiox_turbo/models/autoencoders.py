...
class _DACVAEEncodeModule(nn.Module):
    from typing import Union

    def __init__(self, vae_model, latent_out_proj: Union[nn.Module, None] = None):
        super().__init__()
        ...
        # self.out_proj = latent_out_proj if latent_out_proj is not None else nn.Identity()
        # import pdb; pdb.set_trace()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() == 2:
            x = x.unsqueeze(0)
        assert x.dim() == 3, f"DACVAE encoder expects (B, C, T), got {tuple(x.shape)}"

        target_device = x.device if (x.is_cuda or x.device.type != 'cpu') else next(self.parameters()).device
        self.vae_model.to(device=target_device)

        x = x.to(device=target_device, dtype=torch.float32)
        x = self.vae_model.preprocess(x, self.vae_model.sample_rate)
        # import pdb; pdb.set_trace()
        with torch.no_grad():
            # import pdb; pdb.set_trace()
            latents, codes, raw_latents, commitment_loss, codebook_loss = self.vae_model.encode(x)
...
class _DACVAEDecodeModule(nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        if z.dim() == 2:
            z = z.unsqueeze(0)
        assert z.dim() == 3, f"DACVAE decoder expects (B, D, T), got {tuple(z.shape)}"

        target_device = z.device if (z.is_cuda or z.device.type != 'cpu') else next(self.parameters()).device
        self.vae_model.to(device=target_device)

        z = z.to(device=target_device, dtype=torch.float32)
        with torch.no_grad():
            # import pdb; pdb.set_trace()
            audio = self.vae_model.decode(z)
...
def create_autoencoder_from_config(config: Dict[str, Any]):
    ...
    # import pdb;pdb.set_trace()
    if ae_config.get("encoder", {}).get("type") == "dac-vae" and ae_config.get("decoder", {}).get("type") == "dac-vae":
        # --- 这是新的、正确的路径 ---
        # print("检测到共享的 DAC-VAE 结构，进行一次性加载。")
        
        from .dac_vae.model.dac import DAC # 确保这个 import 路径对你的项目是正确的

        dac_config = ae_config["encoder"]["config"]
        model_path = dac_config.get("model_path")
        
        if model_path is None:
            raise ValueError("在 DAC-VAE 配置中找不到 'model_path'")

        # print(f"从 '{model_path}' 加载共享的 VAE 模型...")
        shared_vae_model = DAC.load(model_path).eval()
        shared_vae_model.requires_grad_(False)

        # 直接使用核心模块 _DACVAEEncodeModule 和 _DACVAEDecodeModule
        # 并将共享的 VAE 实例传递给它们
        encoder = _DACVAEEncodeModule(shared_vae_model)
        decoder = _DACVAEDecodeModule(shared_vae_model)
        # import pdb;pdb.set_trace()
        print("Encoder 和 Decoder 已成功共享同一个 VAE 实例。")
