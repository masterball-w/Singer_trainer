...
class MAF_Block(nn.Module):
    """Multimodal Adaptive Fusion block.

    A unified pool of experts cross-attends to the gated concatenation of the
    video / text / audio tokens, is refined by a shared fusion transformer, and
    then split back into per-modality refinement signals that are added to the
    original tokens through learnable bypass gates.
    """

    def __init__(self, dim: int, num_experts_per_modality: int, num_heads: int,
                 num_fusion_layers: int, mlp_ratio: float = 4.0):
        super().__init__()
        self.dim = dim
    ...
    def _init_weights(self, m: nn.Module):
        # Zero-init the cross-attention output projection to stabilize early training.
