...
from .dit import DiffusionTransformer, MMDiffusionTransformer, QwenMMDiTDiffusionTransformer
...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def __init__(
        ...
        if gate:
            if gate_type == "MAF":
                from .MAF import MAF_Block
                num_experts_per_modality = gate_type_config.get("num_experts_per_modality", 64)
                num_heads = gate_type_config.get("num_heads", 12)
                num_fusion_layers = gate_type_config.get("num_fusion_layers", 4)
                self.maf_block = MAF_Block(dim=768, num_experts_per_modality=num_experts_per_modality, num_heads=num_heads, num_fusion_layers=num_fusion_layers)
            else:
...
class MMDiTWrapper(ConditionedDiffusionModel):
    def __init__(
        ...
        **kwargs
        ...
        self.model = MMDiffusionTransformer(*args, **kwargs)
...
class QwenMMDiTWrapper(ConditionedDiffusionModel):
    def __init__(
        self,
        *args,
        **kwargs
        ...
        self.model = QwenMMDiTDiffusionTransformer(*args, **kwargs)
...
def create_diffusion_cond_from_config(config: tp.Dict[str, tp.Any]):
    ...
    elif diffusion_model_type == 'qwen_mmdit':
        diffusion_model = QwenMMDiTWrapper(**diffusion_model_config)
    ...
    elif diffusion_model_type == "dit" or diffusion_model_type == "mmdit":
    ...
    if model_type == "diffusion_cond" or model_type == "diffusion_cond_inpaint" or model_type == "diffusion_dmd": # Added diffusion_dmd to the conditional wrapper logic
