...
#         t_start = offset / (upper_bound + self.n_samples) # At least the frame correspond to 47s, or longer
#         t_end = (offset + self.n_samples) / (upper_bound + self.n_samples) # t_end correspond to the 47s
...
class PadCrop_Normalized_T(nn.Module):
    ...
    def __call__(self, source: torch.Tensor) -> Tuple[torch.Tensor, float, float, int, int, torch.Tensor]:
        ...
        if self.randomize and n_samples > self.n_samples:
            # Calculate valid start points that are multiples of 10 seconds
            # valid_offsets = [i * self.sample_rate for i in range(0, total_duration-5, 10) if i * self.sample_rate + self.n_samples <= n_samples]
            # valid_offsets = [
            #     i * self.sample_rate for i in range(0, total_duration, 10) 
            #     if i * self.sample_rate + self.n_samples <= n_samples and 
            #     total_duration - i >= 10  # 确保剩余时间至少为10秒
            # ]
            valid_offsets = [
                i * self.sample_rate for i in range(0, total_duration, 10) 
                if i * self.sample_rate + self.n_samples <= n_samples and 
                (total_duration <= 20 or total_duration - i >= 15)  # 根据 total_duration 的值进行判断
            ]
            if valid_offsets:
                offset = random.choice(valid_offsets)
        # print(f'total_duration:{total_duration}')
        # print(f'valid_offsets:{valid_offsets}')
...
# ---------------------------------------------------------------------------
# Video / audio I/O helpers for multimodal (V2A / V2M / TV2A) inference.
# These mirror the helpers used by the Gradio demo so that the README example
# and any user script can preprocess inputs with a single import:
#
#   from audiox_turbo.data.utils import (
#       read_video, load_and_process_audio,
#       encode_video_with_synchformer, merge_video_audio,
#   )
# ---------------------------------------------------------------------------

# Default Synchformer checkpoint (override with AUDIOX_TURBO_SYNCHFORMER_CKPT).
SYNCHFORMER_CKPT = "checkpoints/synchformer/synchformer_state_dict.pth"
...
def read_video(filepath, seek_time=0., duration=-1, target_fps=2):
    """Read a video (or image) into a [N, C, 224, 224] uint8 tensor at target_fps."""
    import os
    ...
    if ext in ['.jpg', '.jpeg', '.png']:
        ...
        frame = frame.repeat(int(_math.ceil(target_frames / frame.shape[0])), 1, 1, 1)[:target_frames]
    ...
    if duration > 0:
        ...
        frame_interval = int(_math.ceil(fps / target_fps))
...
def load_and_process_audio(audio_path, sample_rate, seconds_start, seconds_total):
    """Load an audio prompt, crop/pad it to seconds_total. Returns a zero tensor if None."""
    import torchaudio
...
def _get_sync_feature_extractor(synchformer_ckpt=None, device="cuda"):
    """Lazily build the Synchformer feature extractor (kept off the import path)."""
    ...
    if _sync_feature_extractor is None:
        ...
        ckpt = synchformer_ckpt or os.environ.get("AUDIOX_TURBO_SYNCHFORMER_CKPT", SYNCHFORMER_CKPT)
        if not os.path.exists(ckpt):
            raise FileNotFoundError(
                f"Synchformer checkpoint not found: {ckpt}. Download it into "
                "checkpoints/synchformer/ or set AUDIOX_TURBO_SYNCHFORMER_CKPT."
            )
        _sync_feature_extractor = FeaturesUtils(
            synchformer_ckpt=ckpt,
            enable_conditions=True,
        ).eval().to(device)
    return _sync_feature_extractor


def encode_video_with_synchformer(video_path, seconds_start=0, seconds_total=10,
                                  device="cuda", synchformer_ckpt=None):
    """Extract Synchformer sync features used by the video conditioner.

    Returns a [1, T, 768] tensor on ``device``.
    """
    ...
    extractor = _get_sync_feature_extractor(synchformer_ckpt, device)
    ...
    with torch.no_grad():
        video_sync_frames = extractor.encode_video_with_sync(sync_video)
    return video_sync_frames


def merge_video_audio(video_path, audio_path, output_path, start_time, duration):
    """Mux the generated audio onto the source video with ffmpeg."""
